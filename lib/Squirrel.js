...
import { GSDNotifi } from './lib/GSDNotifi.js';
...
export class Squirrel
{
    ...
    constructor(dispatchedEvent, config = defaultConfig)
    {
        ...
        this._notifi = new GSDNotifi(dispatchedEvent);
        this.config = config;
    }
    ...
    async fetch(url, tries = 0)
    {
        ...
        if (tries > 2 || tries < 0) return null;
    ...
    /**
     * Format task data for sending to the API
     */
    async formattedTaskData(obj_task = null)
    {
        const task = obj_task ? {...obj_task} : {...this._task};
        delete task._manager;
        delete task.dispatcher;
        task.user_uuid = task?.user_id ?? null;
        task.data = obj_task?.data || {};
        return task;
    }
}
